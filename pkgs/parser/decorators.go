...
// IsKnownFunctionDecorator checks if a name is a known function decorator
func IsKnownFunctionDecorator(name string) bool {
	return StandardDecorators.IsFunctionDecorator(name)
}

// IsKnownBlockDecorator checks if a name is a known block decorator
func IsKnownBlockDecorator(name string) bool {
	return StandardDecorators.IsBlockDecorator(name)
}

// MustHaveExplicitBlock checks if a decorator must have explicit braces
func MustHaveExplicitBlock(name string) bool {
	return StandardDecorators.RequiresBlock(name)
}

// ValidateDecoratorUsage validates that a decorator is used correctly
func ValidateDecoratorUsage(name string, args []string, hasBlock bool) error {
