...
func TestBlockDecorators(t *testing.T) {
	testCases := []TestCase{
		...
		},
		{
			...
			Expected: Program(
				CmdBlock("dangerous",
					Decorator("confirm", Str("Are you sure?")),
					Text("rm -rf /tmp/*"),
				),
			),
		},
...
func TestDecoratorVariations(t *testing.T) {
	testCases := []TestCase{
		...
		},
		{
			...
			Expected: Program(
				CmdBlock("advanced",
					Decorator("watch-files", Id("PATTERN")),
					Text("rebuild"),
				),
			),
		},
