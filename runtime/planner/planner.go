...
func (p *planner) planShellCommand() (planfmt.Step, error) {
	...
	// INVARIANT: depth must be 0 (balanced events)
	if depth != 0 {
		return planfmt.Step{}, &PlanError{
			Message:     "unbalanced shell command events",
			Context:     "parsing shell command",
			EventPos:    startPos,
			TotalEvents: len(p.events),
			Suggestion:  "This is likely a parser bug - please report it",
		}
	}
		...
		Commands: []planfmt.Command{
			{
				...
				Operator: "", // Last (and only) command in step
			},
		},
	}

	if p.config.Debug >= DebugDetailed {
		p.recordDebugEvent("step_created", fmt.Sprintf("id=%d decorator=@shell command=%q", step.ID, command))
	}

	// POSTCONDITION: position must advance
	invariant.Postcondition(p.pos > startPos, "position must advance in planShellCommand")
